# agents.py — deployment version (flat structure, st.secrets support)
...
# ── Paths (flat: stocks.db lives next to this file) ───────────
...
def run_baseline(question: str, model: str, client=client, verbose: bool = False) -> AgentResult:
    ...
    return run_specialist_agent(
        agent_name="Baseline", system_prompt=system_prompt, task=question,
        tool_schemas=[], model=model, client=client, verbose=verbose,
    )
...
def run_single_agent(question: str, model: str, client=client, verbose: bool = False) -> AgentResult:
    return run_specialist_agent(
        agent_name="SingleAgent", system_prompt=SINGLE_AGENT_SYSTEM_PROMPT, task=question,
        tool_schemas=ALL_SCHEMAS, model=model, client=client, verbose=verbose,
    )
