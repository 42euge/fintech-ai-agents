...
try:
    ...
    _openai_key = _st.secrets.get("OPENAI_API_KEY", "")
except Exception:
    import os
    _openai_key = os.getenv("OPENAI_API_KEY", "")

if not _openai_key:
    st.error("OPENAI_API_KEY not set. Add it in the Streamlit Cloud secrets dashboard.")
    st.stop()

if not (pathlib.Path(_MODULE_DIR) / "stocks.db").exists():
    st.warning("stocks.db not found — SQL tool calls will fail.")
...
def build_question_with_history(history: list, current_question: str) -> str:
    MAX_TURNS = 6
...
def call_agent(question: str, architecture: str, model: str) -> tuple:
